  ...
  setupMainRenderer() {
    ...
    this.mainRenderer.image = function(href, title, text) {
      ...
      if (!href.match(/^https?:\/\//)) {
        ...
        let caption = href.toString().split(/[\/\\]/)
          caption = caption[caption.length -2] + " " + caption[caption.length -1].replace(/\..*/,"");
        return util.format('<figure style="text-align: center"><img src="%s" /><figcaption style="text-align: center">Figure: %s</figcaption></figure>', datauri(href), caption)
      } else {
          let caption = href.toString().split(/[\/\\]/)
          caption = caption[caption.length -2] + " " + caption[caption.length -1].replace(/\..*/,"");
          console.log("href:", caption);
        return util.format('<figure style="text-align: center"><img src="%s" /><figcaption style="text-align: center">Figure: %s</figcaption></figure>', href, caption)
      }
    }
    return this
  }
